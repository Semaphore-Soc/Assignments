...
//Barrier
struct barrier barrier_array[BARRIER_ARRAY_SIZE];

//Buffer
struct buffer_cond_elem buffer_cond[BUFFER_SIZE];
...
//Semaphore buffer
int buffer_sem[BUFFER_SIZE];
struct semaphore pro, con, empty, full;
int nextp = 0, nextc = 0;
...
// Reacquires lock when awakened.
void
condsleep(cond_t *chan, struct sleeplock *lk)
{
  struct proc *p = myproc();
  uint xticks;
...
// Wakes up one of the processe (if any) sleeping on condition variable (chan).
// Must be called without any p->lock.
void
wakeupone(void *chan)
{
...
int barrier_alloc(void)
{
  for(;;)
  {
    ...
    {
      acquiresleep(&(barrier_array[i].lk));
      if ((barrier_array[i].n) == -1)
      {
        barrier_array[i].n = 0;
        releasesleep(&(barrier_array[i].lk));
        return i;
      }
      releasesleep(&(barrier_array[i].lk));
    }
...
void barrier(int instance, int id, int num_process)
{
  acquiresleep(&(barrier_array[id].lk));
  (barrier_array[id].n)++;
  printf("%d: Entered barrier#%d for barrier array id %d\n", myproc()->pid, instance, id);
  if ((barrier_array[id].n) == num_process)
  {
    barrier_array[id].n = 0;
    cond_broadcast(&(barrier_array[id].cv));
  }
  else
    cond_wait(&(barrier_array[id].cv), &(barrier_array[id].lk));
  printf("%d: Finished barrier#%d for barrier array id %d\n", myproc()->pid, instance, id);
  releasesleep(&(barrier_array[id].lk));
  return;
}
...
void buffer_cond_init(void)
{
  for (int i = 0; i < BUFFER_SIZE; i++)
  {
    ...
    initsleeplock(&buffer_cond[i].lk, "buffer lock");
  }
  head=0;
  tail=0;
...
void cond_produce(int produce)
{
  ...
  acquiresleep(&buffer_cond[index].lk);
  while (buffer_cond[index].full)
    cond_wait(&buffer_cond[index].deleted, &buffer_cond[index].lk);
  buffer_cond[index].x = produce;
  buffer_cond[index].full = 1;
  cond_signal(&buffer_cond[index].inserted);
  releasesleep(&buffer_cond[index].lk);
}
...
int cond_consume(void)
{
  ...
  acquiresleep(&buffer_cond[index].lk);
  while (!buffer_cond[index].full)
    cond_wait(&buffer_cond[index].inserted, &buffer_cond[index].lk);
  v = buffer_cond[index].x;
